...
def set_sound(date_time: datetime, name: str, pre: bool):
    ...
    if len(overrides) == 0:
        ...
        for ring_time in timetable_today:
            if pre == False:
                ...
                cursor.execute(f"""SELECT presound FROM {table} WHERE time={str(date_time.hour).zfill(2)}:{str(date_time.minute).zfill(2)}""")
